...
## 웹 어플리케이션을 서비스 공개해 보자

퍼블릭 클라우드인 GCP의 가상 머신, GCE를 사용해 인터넷 상에 서비스를 공개한다.

이 때 가상 머신 내의 Docker 실행 환경을 로컬의 Docker Machine으로 구축하는 과정.

### Docker 실행 환경 작성

GCP에 프로젝트를 생성해둔다. GCE API의 사용을 허가받아둔다.

로컬에서는 Google Cloud SDK를 설치해둔다.

`docker-machine create`로 시작하는 명령을 통해 특정 클라우드의 특정 프로젝트를 지목해, 원하는 사양의 Docker 실행 환경을 구축할 수 있다.

- 지목하는 클라우드가 무엇인지에 따라 지정하는 옵션값이 달라진다.

#### GCP 관련 옵션

##### --driver 옵션

GCE상에 실행 환경을 구축할 것이므로, google이라는 값을 넣는다.

##### --google-project 옵션

GCP는 **프로젝트**라는 단위로 리소스를 관리하므로, 미리 생성해둔 프로젝트의 ID를 입력한다.
...
타입에 따라 가상 CPU, 메모리 크기, 무엇보다도 시간당 요금이 달라진다.

##### --google-tags 옵션

인스턴스에 설정할 태그를 지정한다.

외부에 HTTP를 열고자 한다면, `http-server` 태그를 지정할 필요가 있다.

##### --google-machine-image 옵션

인스턴스의 머신 이미지를 지정한다. 흔히 Ubuntu의 LTS 버전을 지정한다.

#### Docker 실행 환경 확인

제대로 실행되었다면,

`docker-machine ls`로 로컬에서 확인할 수 있고,

GCP의 콘솔에서도 확인 가능하다.

### 웹 어플리케이션 전개

docker-machine 명령을 통해, 인스턴스에 **ssh 접속**이 가능하다.

Docker 실행 환경은 이미 구축되어 있으므로, Docker 레지스트리에서 만들어둔 이미지를 가져와서 구동시키면 된다.

### Docker 실행 환경 삭제

`docker-machine rm`으로 삭제하면 클라우드 상의 인스턴스가 날라간다.
