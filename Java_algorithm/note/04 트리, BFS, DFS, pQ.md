...
원소들 간의 계층관계가 존재하여, 상위 원소에서 하위 원소로 내려갈수록 확장되는 나무 모양이다.
...
트리 자체가, **한 개 이상의 노드로 이루어진 유한집합**이다.

### 루트

노드 중 최상위 노드를 **루트**라고 한다.

- 루트만 찾으면 전체 트리를 찾는 게 이론상 가능하다. `LinkedList`의 **헤드**와 동일하다.

### 서브트리

루트에 연결된 부분집합을 끊어내면, 그 자체가 새로운 루트를 가지는 새로운 트리가 된다.

- 재귀적 정의로 볼 수 있다. 

어떤 노드에서 부모 노드와 연결된 간선을 끊었을 때 생성된다.

### 리프노드

각 노드의 가장 끝에 있는 노드를 leaf node, 단말노드라고 한다.

루트에서 leaf까지 가는 거리는, 당연히 트리마다 제각각이 된다.
...
Sibling node. 같은 부모 노드를 공유하는 자식 노드들 간의 관계

### 조상 노드

한 노드에서 간선을 따라 루트 노드까지에 이르는 경로에 있는 모든 노드들
...
노드의 차수: 각 노드에 직접 연결된 자식 노드의 수 = 각 노드 하방에 직접 달려있는 간선의 수

- 단말 노드의 경우: 차수가 0

트리의 차수: 트리에 있는 노드의 차수 중 가장 큰 값

- 이진 트리의 경우: 모든 노드의 차수가 최대 2 이하인 트리
...
4. ● 우선순위 큐를 구현하는 가장 효율적인 방법이, 힙을 사용하는 것.
