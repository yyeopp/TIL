...
import com.CompanionNuri.ProjData;
import com.util.DBUtil;

public class NuriPark {
	...
	private static int findRegionCode(String regionName) {
		...
		try {
			...
			sb.append("select regionCode \n");
			sb.append("from region \n");
			sb.append("where regionName = ?");
...
//
//		System.out.println(temp[1]); // 이름
//		System.out.println(temp[11]); // 지역이름
//		System.out.println(temp[12]); // 주소
//		System.out.println(temp[14]); // 전화번호
//		System.out.println(temp[17]);		// lng는 x좌표, 큰거
//		System.out.println(temp[18]);		// lat y좌표 작은거

		while (!read.equals("#")) {

			String[] data = read.split("\",\"");
//			System.out.println(data[2]);
//			ProjData proj = getDataFromKakao(data[12]);
			...
			int regionCode = findRegionCode(data[11]);
			...
			try {
				...
				StringBuilder sb = new StringBuilder();
				sb.append(
						"insert into location(locListId, locName, locRoadAddr, locTel, locLat, locLng, locRegionCode) \n");
				sb.append("values (?,?,?,?,?,?,?)");
				pstmt = conn.prepareStatement(sb.toString());
